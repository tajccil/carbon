...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const { email, code, redirectTo, intent } = validation.data;

  if (intent === "login") {
    const authSession = await verifyLoginOtpAndCreateAuthSession(email, code);

    if (!authSession) {
      return data(
        error(null, "Invalid or expired sign-in code"),
        await flash(request, error(null, "Invalid or expired sign-in code"))
      );
    }
    ...
    <>
      ...
      <div className="rounded-lg md:bg-card md:border md:border-border md:shadow-lg p-8 w-[380px]">
        <ValidatedForm
          ...
          defaultValues={{ email, redirectTo, intent }}
          method="post"
        >
